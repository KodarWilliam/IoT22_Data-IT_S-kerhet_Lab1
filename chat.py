...
class Chat:
    def __init__(self, username, room):
        ...
        self.f = Fernet(base64.urlsafe_b64encode(bytes("my32lengthsupersecretnooneknows1", 'utf-8')))
    ...
    def on_message(self, client, userdata, message):
        ...
        """message decrypt"""
        try:
            unencrypted_message = self.f.decrypt(message.payload)
        except cryptography.fernet.InvalidToken:
            print("Whops! unencrypted message")
        if not self.username:
            print(f"Topic: {message.topic}, Message: {unencrypted_message.decode('utf-8')}")
    ...
    def init_client(self):
        ...
        join_str = bytes(f'<{self.username}> has joined the chatroom.', 'utf-8')
        join_encrypt = self.f.encrypt(join_str)
        self.client.publish(self.topic, join_encrypt)
        ...
        while True:
            try:
                ...
                if msg_to_send.lower() == "quit":
                    bytes_to_send = bytes(f'<{self.username}> {msg_to_send}.', 'utf-8')
                    left_str = bytes(f'<{self.username}> has left the chatroom.', 'utf-8')
                    left_encrypt = self.f.encrypt(left_str)
                    self.client.publish(self.topic, left_encrypt)
                    # Indicate to the input thread that it can exit
                    self.running = False
                    break
                """encrypt msg_to_send"""
                bytes_to_send = bytes(f'<{self.username}> {msg_to_send}.', 'utf-8')
                encrypted_msg = self.f.encrypt(bytes_to_send)
